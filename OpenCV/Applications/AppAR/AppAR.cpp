...
int AppAR::run(int argc, char* argv[])
{
    ...
    if (INPUT_FILE_TYPE == 0)
    {
        ...
        Mat img_scene;
        if (img_scene_read.cols > (int)MAX_PROCESSING_IMAGE_WIDTH)
        {
    ...
    else if (INPUT_FILE_TYPE == 1)
    {
        ...
        unsigned int frame_count = (unsigned int)(capture.get(CV_CAP_PROP_FRAME_COUNT));
        printf("-- frame count: %d\n", frame_count);

        unsigned int frame_width = (unsigned int)(capture.get(CV_CAP_PROP_FRAME_WIDTH));
        unsigned int frame_height = (unsigned int)(capture.get(CV_CAP_PROP_FRAME_HEIGHT));
        ...
        while (capture.read(frame))
        {
            ...
            if (img_scene_read.cols > (int)MAX_PROCESSING_IMAGE_WIDTH)
            {
                resize(img_scene_read, img_scene, cvSize((int)MAX_PROCESSING_IMAGE_WIDTH, (int)(MAX_PROCESSING_IMAGE_WIDTH * img_scene_read.rows / img_scene_read.cols)));
            }
            else
            {
                img_scene_read.copyTo(img_scene);
...
void AppAR::process(const Mat& img_object, const Mat& img_scene)
{
    ...
    if (min_dist < 0.15)
    {
        ...
        if (DO_SHOW_INTERNAL_STEP_IMAGES)
        {
            ...
            line(img_matches, scene_corners[0] + Point2f((float)img_object.cols, 0.0f), scene_corners[1] + Point2f((float)img_object.cols, 0.0f), Scalar(0, 255, 0), 4);
            line(img_matches, scene_corners[1] + Point2f((float)img_object.cols, 0.0f), scene_corners[2] + Point2f((float)img_object.cols, 0.0f), Scalar(0, 255, 0), 4);
            line(img_matches, scene_corners[2] + Point2f((float)img_object.cols, 0.0f), scene_corners[3] + Point2f((float)img_object.cols, 0.0f), Scalar(0, 255, 0), 4);
            line(img_matches, scene_corners[3] + Point2f((float)img_object.cols, 0.0f), scene_corners[0] + Point2f((float)img_object.cols, 0.0f), Scalar(0, 255, 0), 4);
        }
    }
...
void AppAR::step_clock_start()
{
    clock_step_begin = clock();
}

void AppAR::step_clock_stop()
{
...
void AppAR::readme()
{
    if (INPUT_FILE_TYPE == 0)
    {
        std::cout << " Usage: AppAR <objectImage> <sceneImage>" << std::endl;
    }
    else if (INPUT_FILE_TYPE == 1)
    {
        std::cout << " Usage: AppAR <objectImage> <inputVideo>" << std::endl;
    }
}
